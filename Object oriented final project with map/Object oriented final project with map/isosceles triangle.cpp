...
// Translate the triangle by a certain (x, y)
void IsoscelesTriangle::translate(const double x_translation, const double y_translation)
{
	for (int i{ 0 }; i < 3; i++)
	{
...
void polygonSpace::IsoscelesTriangle::rescale(const double enlargement)
{
	// First find centre point of triangle (taken to be centre of mass)
	...
	x_centre_of_mass = x_coords_sum / 3.0;
	y_centre_of_mass = y_coords_sum / 3.0;
...
// Rotate the triangle
void polygonSpace::IsoscelesTriangle::rotate(const double angle)
{
	...
	// Now find centre point of triangle (taken to be centre of mass)
